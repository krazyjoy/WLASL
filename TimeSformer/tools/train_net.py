...
def plot_metrics(mode, title, loss_history, acc_history):
    if mode == 'Train' or mode == 'train':
        plt.figure(figsize=(21, 8))
        plt.plot(range(1, len(loss_history)+1), loss_history, label=f"{mode} Loss")   # loss_history, label=f"{mode} Loss"
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.title(f"{mode} Loss over time")
        plt.legend(fontsize="15")
        plt.grid(True)
        plt.savefig(f"./metrics/{title}/{mode}_loss.png") 
        plt.close()
    
    plt.figure(figsize=(21, 8))
    plt.plot(acc_history, label=f'{mode} Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy (%)')
    plt.title(f'{mode} Accuracy over time')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"./metrics/{title}/{mode}_acc.png")
...
def build_trainer(cfg):
    ...
    print(f"[DEBUG] train_loader dataset size: {train_loader.dataset}")
...
def train(cfg):
    ...
    model = build_model(cfg, cfg.GPU_ID)
    if du.is_master_proc() and cfg.LOG_MODEL_INFO:
        misc.log_model_info(model, cfg, use_train_input=True)
    ...
    # store entire accuracy and loss history
    train_loss_history = []
    train_acc_history = []
    val_acc_history = []
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        # Train for one epoch.
        epoch_loss_history, epoch_acc_history = train_epoch(
            train_loader, model, optimizer, train_meter, cur_epoch, cfg, writer
        )

        train_loss_history.append(sum(epoch_loss_history)/len(epoch_loss_history))
        train_acc_history.append(sum(epoch_acc_history)/len(epoch_acc_history))
        print(f"train_loss_history: {train_loss_history}")
        print(f"train_acc_history: {train_acc_history}")
        plot_metrics(mode='Train', title='WLASL2000', loss_history=train_loss_history, acc_history=train_acc_history)
        ...
        # if is_eval_epoch:
        if True:
            epoch_acc_history = eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer)
            val_acc_history.append(sum(epoch_acc_history)/len(epoch_acc_history))
            print(f"val_acc_history: {val_acc_history}")
            plot_metrics(mode='Val', title='WLASL2000', loss_history=[], acc_history=val_acc_history)
