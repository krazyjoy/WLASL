...
def load_rgb_frames_from_video(vid_root, vid, start, num, resize=(256, 256)):
    ...
    for offset in range(min(num, int(total_frames - start))):
        ...
        # add to resolve program continues to red frames even though it has reache the end of the video (case when video is too short: https://github.com/dxli94/WLASL/issues/42)
        if not success:
            break
