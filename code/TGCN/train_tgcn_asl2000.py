...
def run(split_file, pose_data_root, configs, subset, save_model_to=None):
    ...
    train_data_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=configs.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    ...
    val_data_loader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=configs.batch_size, shuffle=True, num_workers=4, pin_memory=False)
    ...
    checkpoint = torch.load("/home/chuan194/work/roboticVision/WLASL/code/TGCN/checkpoints/asl2000/gcn_epoch=9_val_acc=0.06009030913511636.pth", map_location=device)
    ...
    df = pd.read_csv("/home/chuan194/work/roboticVision/WLASL/code/TGCN/output/tgcn_training_log_asl2000.csv")
    ...
    best_test_acc = 0
    # start training
    for epoch in range(11, int(epochs)):
        # train, test model
